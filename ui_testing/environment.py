...
def build_default_paths() -> Paths:
    ...
    _ensure_dirs(paths.scripts_dir, paths.images_dir, paths.results_dir)
    paths.test_plan = _find_default_test_plan(app_dir)
    return paths
...
def _find_default_test_plan(root: Path) -> Optional[Path]:
    ...
    prioritized = [
        ...
        if "Test Procedure" in candidate.name or "Explosive" in candidate.name
    ]
